...
def parse_command_line():
    ...
    parser.add_argument("-d",'--date',nargs = 3,metavar = ('month','day','year'),type = int,help = 'formatted date (i.e. 03 28 1998)')
...
def create_date(datelist, surprise):
    ...
    if datelist is not None:
        try:
            Date = date(month = datelist[0],day = datelist[1],year = datelist[2])
            return Date
        except Exception as ex:
            print('error in date object creation',ex)
            return None
    elif surprise:
        try:
            earliest = date(month = 6,day = 16,year = 1995)
            delta = date.today()-earliest
            secs = delta.total_seconds()
            secs = randint(0,secs)
            delta = timedelta(seconds = secs)
            new_date = earliest + delta
            return new_date
        except Exception as ex:
            print('error in random date',ex)
            return None
    else:
...
def save_image(d, image):
    ...
    try:
            ...
            #raise Exception("Path not correct:",os.getcwd())
        ...
        print('error with year and month directories',ex)


    path = "{}-{}-{}.jpg".format(year,month,day)
    try:
        with open(path,'wb') as file:
            file.write(image)
        return os.path.abspath(path)
    except Exception as ex:
        print('error with writing image to file',ex)
